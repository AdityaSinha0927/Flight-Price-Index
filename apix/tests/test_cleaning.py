...
def add_raw(path: Path, *, source="indigo", route="DEL-BOM", search_date="2024-01-01",
            travel_date="2024-01-02", window="T1", total=100.0, base=None,
...
def test_exact_duplicates_keep_only_latest_scrape(raw_db: Path):
    first_id = add_raw(raw_db, total=100.0, scraped_at="2024-01-01T10:00:00+00:00")
    second_id = add_raw(raw_db, total=125.0, scraped_at="2024-01-01T11:00:00+00:00")
...
def test_iqr_outlier_is_flagged_but_retained(raw_db: Path):
    for day, fare in enumerate((100.0, 100.0, 100.0, 1000.0), start=1):
    ...
    assert next(row for row in rows if row[3] == 1000.0)[4] == 1
