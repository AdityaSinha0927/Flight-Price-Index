...
- Align the MoSPI and APIx values by year-month using only overlapping
  months, then independently rebase both series to 100 at the first
  overlapping month.
- Plot both lines with clear labels such as `APIx monthly index (rebased)` and
  `MoSPI airfare CPI (rebased)`. The title/caption must state that this is a
  rebased trend comparison, not an exact index-level match.
